...
void SetExpansionMemType(const SS_CARDTYPE type)
{
	...
	g_CardMgr.Insert(0, newSlot0Card);
...
static void ResetPaging(BOOL initialize)
{
	...
	if (IsApple2PlusOrClone(GetApple2Type()) && g_CardMgr.QuerySlot(0) == CT_Empty)
		SetMemMode(0);
	else
...
void MemInitializeCustomF8ROM(void)
{
	...
	if (IsApple2Original(GetApple2Type()) && g_CardMgr.QuerySlot(0) == CT_LanguageCard)
	{
		try
		{
...
void MemInitializeIO(void)
{
	...
	else if (g_CardMgr.QuerySlot(4) == CT_Z80)
	{
		ConfigureSoftcard(pCxRomPeripheral, 4);		// $C400 : Z80 card
	}
//	else if (g_CardMgr.QuerySlot(4) == CT_GenericClock)
//	{
...
//	}
	...
	else if (g_CardMgr.QuerySlot(5) == CT_SAM)
	{
