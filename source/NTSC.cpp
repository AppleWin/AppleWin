...
static void set_csbits()
{
	...
	case A2TYPE_BASE64A:		csbits = &csbits_base64a[Video::GetVideoRomRockerSwitch() ? 0 : 1]; g_nVideoCharSet = 0; break; // Apple ][ clone
...
void NTSC_SetVideoStyle(void)
{
	...
	switch ( Video::g_eVideoType )
	{
		...
		case VT_MONO_CUSTOM:
			...
			r = (Video::g_nMonochromeRGB >>  0) & 0xFF;
			g = (Video::g_nMonochromeRGB >>  8) & 0xFF;
			b = (Video::g_nMonochromeRGB >> 16) & 0xFF;
_mono:
			updateMonochromeTables( r, g, b ); // Custom Monochrome color
			if (half)
