...
void CMouseInterface::OnCommand()
{
	...
	case MOUSE_READ:				// Read
		...
		m_byState &= ~STAT_MOVEMENT_SINCE_READMOUSE;
#ifdef _DEBUG_SPURIOUS_IRQ
		OutputDebugFormat(szDbg, "[MOUSE_READ] Old=%02X New=%02X\n", byOldState, m_byState);
#endif
		break;
	case MOUSE_SERV:
		m_nDataLen = 2;
		...
		{
			uSpuriousIrqCount++;
			OutputDebugFormat(szDbg, "[MOUSE_SERV] 0x%04X Buff[1]=0x%02X, ***\n", uSpuriousIrqCount, m_byBuff[1]);
		}
		else
		{
			OutputDebugFormat(szDbg, "[MOUSE_SERV] ------ Buff[1]=0x%02X\n", m_byBuff[1]);
		}
#endif
...
#ifdef _DEBUG_SPURIOUS_IRQ
		char szDbg[200];
		OutputDebugFormat(szDbg, "[MOUSE EVNT] 0x%02X Mode=0x%02X\n", m_byState, m_byMode);
#endif
	}
}
