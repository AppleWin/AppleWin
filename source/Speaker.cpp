...
static void InitRemainderBuffer()
{
	...
	if ((double)g_nRemainderBufferSize != g_fClksPerSpkrSample)  // An odd precise comparison. It's probably better to use a comparison with defined precision: fabs(A - B) > Epsilon. 
...
BYTE __stdcall SpkrToggle (WORD, WORD, BYTE, BYTE, ULONG nCyclesLeft)
{
  ...
  if (soundtype == SOUND_WAVE)
  {
      ...
      if (g_bQuieterSpeaker)
      {
       ...
       if (g_nSpeakerData == (SPKR_DATA_INIT >> 2)) // Undefined behavior. Check the shift operator '>>. The left operand 'SPKR_DATA_INIT' is negative.
        g_nSpeakerData = ~g_nSpeakerData;
       else
        g_nSpeakerData = SPKR_DATA_INIT>>2; // Undefined behavior. Check the shift operator '>>. The left operand 'SPKR_DATA_INIT' is negative.
      }
      else
      {
