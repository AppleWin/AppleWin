...
void Disk2InterfaceCard::DataLatchReadWOZ(WORD pc, WORD addr, UINT bitCellRemainder)
{
	...
	for (UINT i = 0; i < bitCellRemainder; i++)
	{
		...
		if (!m_latchDelay)
		{
			...
			if (newLatchData)
			{
				LOG_DISK("read skipped latch data: %04X = %02X (PC=%04X)\r\n", floppy.m_byte, m_floppyLatch, regs.pc);
				newLatchData = false;
			}
#endif
...
void Disk2InterfaceCard::NextLSS(BYTE readPulse)
{
	...
	//  A2 = SHIFT/LOAD
	//  A3 = READ/WRITE
	//  A4 = read pulse' (UTAIIe 9-15: "...inverted in the process to form a negative pulse.")
	//  A5 = LSS ROM O4
	//  A6 = LSS ROM O6
	//  A7 = LSS ROM O7
	const BYTE sequence = m_LSSCode & 0xf0;
	BYTE addr = ((sequence >> 5) & 0x01) |
		((m_shiftReg >> 6) & 0x02) |
		(((BYTE)m_seqFunc.loadMode) << 2) |
		(((BYTE)m_seqFunc.writeMode) << 3) |
		((readPulse ? 0 : 1) << 4) |
		((sequence << 1) & 0x20) |
		(sequence & 0xC0);
...
void Disk2InterfaceCard::DataLatchReadWOZFlux(WORD pc, WORD addr, UINT ticks)
{
	...
	for (UINT i = 0; i < ticks; i++)
	{
		...
		floppy.m_tickCountdown--;
		if (floppy.m_tickCountdown == 0)
		{
			floppy.m_bitCellCount--;
			floppy.m_bitOffset++;

			const BYTE floppyBit = floppy.m_bitCellCount ? 0 : 1;
...
#endif
#if 0
				if (drive.m_phasePrecise == 10.0)	// T$05.00
				{
					LOG_DISK("%04X = %02X (PC=%04X)\n", floppy.m_byte, m_floppyLatch, regs.pc);
				}
#endif
			}
		}
