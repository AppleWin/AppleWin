...
namespace
{
    ...
    void parseAddress(
        const addrinfo &hints, const std::string &addr, const std::string &port, in_addr &host_addr, int &host_port)
    {
        ...
        if (r)
        {
            throw std::runtime_error(
                StrFormat("getaddrinfo('%s:%s') [%s]", addr.c_str(), port.c_str(), gai_strerror(r)));
        }
